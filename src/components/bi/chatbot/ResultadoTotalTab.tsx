...
const allColumns: ColumnDef[] = [
  ...
  { key: "profit", label: "Lucro", group: "financial", format: (v) => <span className={v === 0 ? "text-muted-foreground" : v >= 0 ? "text-profit" : "text-loss"}>{formatBRL(v)}</span>, defaultVisible: true },
...
export function ResultadoTotalTab({ campaigns, popEnabled = false, focusMode = false, onToggleFocusMode, filtersNode, tabsListNode }: Props) {
  ...
  const containerClass = "flex flex-col flex-1 min-h-0";
  ...
  return (
    <div className={containerClass}>
      {/* Toolbar */}
      <div className="shrink-0 flex items-center justify-between gap-2 px-1 py-2">
      ...
      <div className="flex-1 overflow-auto relative min-h-0">
        <table className="w-full text-xs border-collapse min-h-full">
